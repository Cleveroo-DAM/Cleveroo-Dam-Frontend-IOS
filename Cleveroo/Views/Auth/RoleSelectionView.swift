...
struct RoleSelectionView: View {
    ...
    @State private var showRegisterParent = false
    
    var body: some View {
        ZStack {
            if showRegisterParent {
                RegisterParentView(viewModel: authViewModel, onBack: {
                    showRegisterParent = false
                })
                .transition(.asymmetric(insertion: .move(edge: .trailing), removal: .move(edge: .leading)))
            } else if let role = selectedRole {
                // Afficher la vue de login pour le rôle sélectionné
                Group {
                    if role == .child {
                        ...
                        ParentLoginView(viewModel: authViewModel, onBack: {
                            selectedRole = nil
                        }, onRegister: {
                            showRegisterParent = true
                        })
                    }
                }
                .transition(.asymmetric(insertion: .move(edge: .trailing), removal: .move(edge: .leading)))
        ...
        .animation(.easeInOut(duration: 0.3), value: showRegisterParent)
        .onChange(of: authViewModel.isLoggedIn) { _ in
            // Réinitialiser la sélection du rôle quand l'utilisateur se déconnecte
            selectedRole = nil
            showRegisterParent = false
        }
    }
