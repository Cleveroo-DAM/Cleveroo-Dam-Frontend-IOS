...
class AuthViewModel: ObservableObject {
    ...
    @Published var isChildRestricted = false // Contrôle parental
    @Published var restrictionReason: String?
    @Published var childId: String? // ID de l'enfant connecté
        ...
        sendRequest(urlString: endpoint, body: body) { [weak self] success, json, error in
            DispatchQueue.main.async {
                ...
                if success, let token = json?["access_token"] as? String {
                    ...
                    // Si c'est un enfant, gérer la restriction
                    if !isEmail {
                        // Récupérer isRestricted et restrictionReason du login
                        self.isChildRestricted = json?["isRestricted"] as? Bool ?? false
                        self.restrictionReason = json?["restrictionReason"] as? String
                        self.childId = json?["childId"] as? String
                        
                        // Sauvegarder l'ID de l'enfant dans UserDefaults pour un accès facile
                        ...
                        if self.isChildRestricted {
                            print("🚫 Child is restricted: \(self.restrictionReason ?? "No reason provided")")
                        } else {
        ...
        sendRequest(urlString: endpoint, method: "GET", token: token) { [weak self] success, json, error in
            DispatchQueue.main.async {
                ...
                if success, let json = json {
                    ...
                    // Car le backend peut retourner false dans le profil même si l'enfant est bloqué
                    if !self.isChildRestricted {
                        self.isChildRestricted = json["isRestricted"] as? Bool ?? false
                    }
                    ...
                    print("   🎂 Age: \(self.age)")
                    print("   🖼️ Avatar URL: \(self.avatarURL ?? "nil")")
                    print("   🚫 Is Restricted: \(self.isChildRestricted)")
                } else {
