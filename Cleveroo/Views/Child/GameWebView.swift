...
struct GameWebView: View {
    ...
    var body: some View {
        Group {
            // Si c'est Mental Math, afficher MentalMathGameView
            if assignment.activityId.type.lowercased() == "mental_math" {
                MentalMathGameView(assignment: assignment, activityVM: activityVM, onComplete: onComplete)
            } else {
                // Sinon, WebView pour external_game
                NavigationStack {
                    ZStack {
                        // WebView
    ...
    // À LA FIN DE GameWebView.swift, REMPLACER le #Preview par :
