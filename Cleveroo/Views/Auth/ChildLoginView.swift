...
struct ChildLoginView: View {
    ...
    var body: some View {
        if showQRCodeLogin {
            ChildQRCodeLoginView(viewModel: viewModel) {
                // Callback pour retourner à ChildLoginView
                withAnimation {
                ...
                ScrollView {
                    VStack(spacing: 25) {
                        ...
                        VStack(spacing: 20) {
                            ...
                            // QR Code Login Button - Uses @State instead of NavigationLink
                            Button(action: {
                                print("✅ QR Code Login button tapped!")
...
#Preview {
    ChildLoginView(viewModel: AuthViewModel(), onBack: {})
}
